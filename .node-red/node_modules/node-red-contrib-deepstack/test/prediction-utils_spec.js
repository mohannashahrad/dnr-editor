...
describe('Prediction Utils', function() {
    describe('#drawPredictions()', function() {
        var deepstack;
        var image;
        var server;
    ...
    describe('#customModel()', function() {
        ...
        it('should POST to /vision/custom/my_model', async function() {
            const scope = nock('http://deepstack.example.com:80')
                .post('/v1/vision/custom/my_model', function(body) {return true})
                .reply(200, {test:"test"});
            
            var res = await deepstack.customModel(image, server, 0.8, 'my_model');
            expect(scope.isDone()).to.be.true;
            expect(res.test).to.equal('test');
        });
        ...
        it('should use protocol from server', async function() {
            ...
            await deepstack.customModel(image, server, 0.8, 'my_model');
            expect(scope.isDone()).to.be.true;
        });
        ...
        it('should use version from server', async function() {
            server.version = 'v2';
            const scope = nock('http://deepstack.example.com:80')
                .post('/v2/vision/custom/my_model', function(body) {return true})
                .reply(200, {test:"test"});
        ...
        it('should set image as image.jpg in the form data', async function() {
            const scope = nock('http://deepstack.example.com:80')
                .post('/v1/vision/custom/my_model', function(body) {
                    return body.includes('Content-Disposition: form-data; name="image"; filename="image.jpg"\r\nContent-Type: image/jpeg')
                })
                .reply(200, {test:"test"});
